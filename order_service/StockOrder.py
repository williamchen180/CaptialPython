...
class Order(Frame):
    ...
    def __CreateWidget(self):
        ...
        # 同步/非同步
        Label(frame, style="Pink.TLabel", text = "同步/非同步").grid(column = 7, row = 0)
            # 輸入框
        boxAsync = Combobox(frame, width = 10, state='readonly')
        boxAsync['values'] = Config.ASYNC
        boxAsync.grid(column = 7, row = 1, padx = 10)
...
class CancelOrder():
    ...
    def __CreateWidget(self):
        ...
        group.grid(column = 0, row = 2,  columnspan = 5,padx = 10, pady = 10, sticky = 'ew')
...
class StockOrder(Frame):
    ...
    def SetID(self, id):
        for _ in 'order', 'decrease', 'cancel','GetRealBalance','GetRequestProfit','GetMarginPurchaseAmountLimit','GetBalanceQ':
